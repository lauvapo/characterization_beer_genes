#!/usr/bin/env python
# BUSCA GENES
# HACE EL MULTIFASTA COMPILATION CON LOS 85 GENOMAS
# CREA LA BASE DE DATOS BLAST CON COMPILATION FASTA
# HACE BLAST CON EL MULTIFASTA DE LOS GENES FRENTE A LA DB COMPILATION
# CAMBIA NÚMEROS DE CROMOSOMAS A ROMANOS
# CREA GFF3
...
for name in n:
    print(f'\nDOING BLAST OF {name}...') ##################### PONER EL PIDENTITY
    command = f'blastn -db compilation_db -query {name}.fa -out results_{name}.out -perc_identity 95 -outfmt "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore"'
    result = subprocess.run(command, shell=True, capture_output=True, text=True)
    command_gff = f'python3 blast2gff.py -b results_{name}.out >> genes.gff3'
...
blast = list()
for i in n:
    subprocess.run(f"cat header.txt results_{i}.out > /blast/{i}_blast.tsv", shell=True)
    subprocess.run(f"rm results_{i}.out", shell=True)
    blast.append(f'{i}_blast.tsv')
